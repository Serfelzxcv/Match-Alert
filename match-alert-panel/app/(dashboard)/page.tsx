...
export default function OverviewPage() {
  ...
  return (
    <>
      <Header
        description="Resumen operativo de partidos, reglas y avisos en vivo"
        title="Panel Match Alert"
      />
      <div className="flex-1 space-y-4 p-4">
        ...
        <div className="flex items-center gap-2 rounded-lg border border-info/20 bg-info/5 px-3 py-2 text-xs">
          ...
          <span>
            {config.communityName} monitorea partidos en vivo y prioriza avisos
              ...
              <CardContent className="px-3">
                ...
                {stat.weekly > 0 ? (
                  <p className="hidden items-center gap-1 text-success text-xs sm:flex">
                    <TrendingUp className="h-3 w-3" />+{stat.weekly} esta semana
                  </p>
                ) : (
        ...
        <Card className="gap-1 py-2.5 transition-colors hover:bg-accent/50 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 sm:gap-1.5 sm:py-3">
          <CardHeader className="flex flex-row items-center justify-between px-3 pb-0">
            ...
            <div className="hidden h-5 w-5 items-center justify-center rounded-full bg-muted sm:flex">
              <BellRing className="h-2.5 w-2.5 text-muted-foreground" />
            </div>
          </CardHeader>
          <CardContent className="px-3">
            <AnimatedNumber
            ...
            {thisWeek.total > 0 ? (
              <p className="hidden items-center gap-1 text-success text-xs sm:flex">
                <TrendingUp className="h-3 w-3" />+{thisWeek.total} esta semana
              </p>
            ) : (
              <p className="hidden text-muted-foreground text-xs sm:block">
                Sin cambios esta semana
      ...
      <CardContent>
        {recent.length === 0 ? (
          <p className="text-muted-foreground text-sm">
            Aun no hay eventos. Las alertas apareceran cuando se conecten los partidos.
          </p>
        ) : (
