...
export function ConversationPreviewToggle() {
  ...
  return (
    <button
      aria-label={
        isOpen ? "Ocultar vista previa" : "Mostrar vista previa"
      }
    ...
    >
      ...
      {(() => {
        ...
        return isOpen ? "Ocultar" : "Vista previa";
      })()}
      {!isPending &&
        (isOpen ? (
...
export function ConversationPreviewContent() {
  ...
  if (messages.length === 0) {
    return (
      <p className="text-muted-foreground text-xs">Sin conversacion registrada.</p>
    );
  }
    ...
    <div className="divide-y rounded-md border bg-muted/30">
      ...
      {messages.length > 4 && (
        <div className="px-3 py-1.5 text-muted-foreground/60 text-xs">
          +{messages.length - 4} mensajes mas
        </div>
      )}
    </div>
  );
