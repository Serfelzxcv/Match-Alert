...
export default function ActivityPage({ searchParams }: PageProps<"/activity">) {
  ...
  return (
    <>
      <Header
        description="Eventos recientes, reglas disparadas y estado de avisos"
        title="Alertas"
      />
    ...
    return (
      <div className="flex flex-col items-center justify-center rounded-lg border border-dashed p-8 text-center">
        ...
        <h3 className="mt-3 font-medium text-base">
          {hasFilters ? "No hay eventos con esos filtros" : "Aun no hay actividad"}
        </h3>
        <p className="mt-1 text-muted-foreground text-sm">
          {(() => {
            if (searchQuery && filterLabel) {
              return `No hay eventos de ${filterLabel.toLowerCase()} para "${searchQuery}".`;
            }
            if (searchQuery) {
              return `No hay eventos para "${searchQuery}". Prueba otra busqueda.`;
            }
            if (filterLabel) {
              return `Aun no hay eventos de ${filterLabel.toLowerCase()}.`;
            }
            return "Los eventos apareceran aqui cuando entren partidos, goles y reglas disparadas.";
          })()}
        </p>
      </div>
    );
...
function ActivityFiltersSkeleton() {
  return (
    <div className="flex flex-wrap gap-1">
      {["Todo", "Goles", "Alertas", "En vivo", "Oportunidades", "Riesgos"].map(
        (label) => (
          <Skeleton
